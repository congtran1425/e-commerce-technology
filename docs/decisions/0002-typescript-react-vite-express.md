...
Frontend và backend cần một hệ công cụ thống nhất. Dự án cũng chia sẻ hợp đồng API giữa hai phía.
...
Phiên bản chính xác được khóa trong `package-lock.json`; môi trường cài đặt lặp lại dùng `npm ci` thay vì tự chọn lại phiên bản.

## Consequences

- FE và BE có kiểm tra kiểu thống nhất và có thể tái sử dụng contract.
- Dependency được cài một lần tại root nhờ npm workspaces.
